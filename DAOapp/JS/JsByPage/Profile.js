...
async function GetVotedProposals(){
    ...
    for(let i = 0; i < VotedProposals.length; i++){
        ...
        UserVo
        let Reward = parseFloat(web3.utils.fromWei(BigInt(CurrentProposalInfo[2].CLDtoIncentive))).toFixed(2) / parseFloat(web3.utils.fromWei(BigInt(CurrentProposalInfo[2].VotesLocked))).toFixed(2) * parseFloat(web3.utils.fromWei(VoterInfo.VotesLocked)).toFixed(2);
        if (IsOver) {
            ProposalStatus = "Over";
            TimeLeft = "Over";
            IsOver = "flex";
