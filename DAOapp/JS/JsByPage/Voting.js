...
async function CheckApproveVotingCLDContitional(){
    ...
    if(approved < 696969696969969696969696969696969669696n){
        gas = await CLDtoken.methods.approve(FlexContractAddress, BigInt(1000000000000000000000000)).estimateGas({from: account});
        await CLDtoken.methods.approve(FlexContractAddress, BigInt(1000000000000000000000000)).send({from: account, value: 0, gas: gas});
    }
}
