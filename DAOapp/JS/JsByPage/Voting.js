...
async function CheckApproveVotingCLDContitional(){
    ...
    if(approved < 696969696969969696969696969696969669696n){
        let gasEstimate = await CLDtoken.methods.approve(VotingAddress, BigInt(696969696969969696969696969696969669696)).estimateGas({ from: account });
        await CLDtoken.methods.approve(VotingAddress, BigInt(696969696969969696969696969696969669696)).send({ from: account, gas: gasEstimate });
    }
}
